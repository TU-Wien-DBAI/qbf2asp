...
namespace qbf2asp
{
        ...
        ConstCollection<vertex_t> children) const
    {
        for (auto child : children) {
            const Qbf2DatalogTable &childTable = table(child, tables);
            for(auto it = remaining.begin(); it != remaining.end();) {
                if(childTable.remaining.find(*it) == childTable.remaining.end()) {
        ...
        htd::ConstIterator<htd::vertex_t> end) const
    {
        ...
        // Find all the eliminated variables
        eliminated.insert(ctable.eliminated.begin(), ctable.eliminated.end());

        // Find all the forgotten variables
